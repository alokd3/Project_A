...
```{r} 
HAMRDT<-read.csv("activity.csv", header=TRUE, colClasses=c("numeric", "Date","numeric"))
``` 
...
We have time series data with a start date is `r as.character(min_date)` and most recent date is `r as.character(max_date)` over a span `r as.character(n_days)` days.
...
```
There are `r as.character(num_compl_cases)` rows with no missing observation and `r as.character(num_incmpl_cases)`  rows with NAs in the data.
...
hist(stps_per_day$steps, main = "Total Number of Steps Per Day",
    ...
    breaks = 10, xlim = c(0, max_num_steps), ylim = c(0, 20), col = "red")
``` 
...
``` 
On an average `r mean_num_steps` steps taken per day and with a meadian of `r median_num_steps` steps per day. Note that I have applied na.rm=TRUE to exclude missing values NA. I have also used format() to convert large numeric numbers separated with commas (",") to give a pretty look.
...
```{r}
avg_no_stps_intvl <- as.data.frame(tapply(compl_HAMRDT$steps,
                                          INDEX = compl_HAMRDT$interval,
                                          FUN = "mean", na.rm = TRUE))
...
num_incmpl_cases<-nrow(in_compl_HAMRDT)
``` 
Total Number of missing rows in the activity monitoring data is `r num_incmpl_cases`.
...
```{r}
median_stps_intvl <- as.data.frame(tapply(compl_HAMRDT$steps,
                                          INDEX = compl_HAMRDT$interval,
...
# Number of steps on a particular date
```{r}
stps_per_day_nm <- as.data.frame(tapply(HAMRDT_No_Miss$steps, INDEX = HAMRDT_No_Miss$date, FUN = "sum"))
